...
map('n', 'go', function() vim.lsp.buf.type_definition() end, { desc = 'Go to type definition' })
map('n', 'gr', function() vim.lsp.buf.references() end, { desc = 'Go to reference' })
map('n', 'gs', function() vim.lsp.buf.signature_help() end, { desc = 'Show function signature' })
map('n', '<F2>', function() vim.lsp.buf.rename() end, { desc = 'Rename symbol' })
map('n', '<F3>', function() vim.lsp.buf.format({ async = true }) end, { desc = 'Format file' })
map('x', '<F3>', function() vim.lsp.buf.format({ async = true }) end, { desc = 'Format selection' })
map('n', '<F4>', function() vim.lsp.buf.code_action() end, { desc = 'Execute code action' })

if vim.lsp.buf.range_code_action then
    map('x', '<F4>', function() vim.lsp.buf.range_code_action() end, { desc = 'Execute code action' })
else
    map('x', '<F4>', function() vim.lsp.buf.code_action() end, { desc = 'Execute code action' })
end
